...
def build_xgboost_classifier(train_data):
    ...
    # feature_names = ['runtimeMinutes', 'numVotes', 'year']
    ...
    print("Area Under ROC (AUC) on training data: {:.4f}".format(auc))
...
def predict_labels(model, df, csv_name):
    ...
    predictions.select("label").write.mode("overwrite").csv("predictions_" + csv_name + ".csv", header=False)
